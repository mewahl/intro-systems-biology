%(3)
global a b c d
%input for 3.a is A=[-2 2; 3 -4]
%input for 3.b is A=[0 -1; 1 1], take the larger eigenvalue
%input for 3.c is A=[0 -1; 2 0]
%input for 3.d is A=[0 -1; 1 1], take the smaller eigenvalue
%input for 3.e is A=[2 2; 3 4]
%input for 3.f is A=[1 2; 3 4]
A=[0 -1; 2 0];

[v,l] = eig(A);
disp(sprintf('The first eigenvector is (%0.4f,%0.4f) with eigenvalue %0.4f', v(1,1), ...
v(2,1), l(1,1)));
disp(sprintf('The second eigenvector is (%0.4f,%0.4f) with eigenvalue %0.4f', v(1,2), ...
v(2,2), l(2,2)));
 a = A(1,1);
 b = A(1,2);
 c = A(2,1);
 d = A(2,2);
 [x, y] = meshgrid(-5:0.5:5, -5:0.5:5);
 dx = a*x + b*y;
 dy = c*x + d*y;
 r = (dx.^2 + dy.^2).^0.5;
 dx = dx ./ r;
 dy = dy ./ r;

 quiver(x,y,dx,dy); hold on;
 xlabel('x(t)')
 ylabel('y(t)')
 axis([-5,5,-5,5])
 [t, e] = ode45(@updater, [0 50], [3, 0]);
 plot(e(:,1),e(:,2),'-r', 'LineWidth', 3)
 [t, e] = ode45(@updater, [0 50], [0,2]);
 plot(e(:,1),e(:,2),'-g', 'LineWidth', 3);
 [t, e] = ode45(@updater, [0 50], [0, 4]);
 plot(e(:,1),e(:,2),'-b', 'LineWidth', 3);
 [t, e] = ode45(@updater, [0 50], [0, 5]);
 plot(e(:,1),e(:,2),'-k', 'LineWidth', 3);
 [t, e] = ode45(@updater, [0 50], [5, 0]);
 plot(e(:,1),e(:,2),'-m', 'LineWidth', 3);